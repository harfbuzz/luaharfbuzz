...
describe("harfbuzz module", function()
  ...
  describe("harfbuzz.Face", function()
    ...
    it("can be initialized with a blob", function()
      ...
      local face = harfbuzz.Face.new_from_blob(blob, 0)
      assert.is_not_nil(face)
    end)

    it("can be initialized with a file and a font index", function()
      local face = harfbuzz.Face.new('fonts/notonastaliq.ttf', 0)
      assert.is_not_nil(face)
      assert.are_equal(0, face:get_index())
    end)
    ...
    it("can return face blob", function()
      local blob = harfbuzz.Blob.new_from_file_or_fail('fonts/notonastaliq.ttf')
      local face = harfbuzz.Face.new_from_blob(blob, 0)
      local b = face:get_blob()
      assert.is_not_nil(b)
      assert.are_equal(blob:get_length(), b:get_length())
    end)
